...
| `REL-OBJ-008` Postmark acceptance and idempotency | Eligible valid webhooks are good within **10s**, availability **≥99.5%**, p95 **≤5s**; **100%** of accepted MessageIDs resolve to one complete Article/Issue. Lost response after commit resolves by replay, with no accepted ID ambiguous >24h. | Rolling 30d, 0.5% availability; zero atomicity/idempotency budget. Use p95 only at ≥100 eligible events/30d; below it, every provider-correlated event uses the 10s maximum and exact failure thresholds. | Sequential dedupe and the required PostgreSQL race now pass: concurrent MessageID lookups converge on one complete Issue, and injected failure rolls back Feed/Article/Issue creation. No provider series, ambiguity state or historical orphan detector exists. | Provider/app correlation by non-secret event ID, outcome/ambiguity/orphan/replay counters, historical orphan audit, and production result/latency evidence. |
