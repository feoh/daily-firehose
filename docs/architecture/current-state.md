...
| [`feeds/recommendations.py`](../../feeds/recommendations.py) | **[post-snapshot]** Per-user all-time recommendation service. Builds bounded text documents from every Article and the authenticated user's SavedArticle history; uses scikit-learn TF-IDF/logistic relevance plus deterministic source/near-duplicate diversity, caches serializable ranked IDs/reasons by corpus/profile/read-state fingerprint, and hydrates only the configured final page. Dates and read state are not ranking features, but effective read state dismisses a candidate. |
...
| GET* | `/recommended/` / `recommended` | session | Complete per-user SavedArticle profile and all unread, unsaved Articles → up to `RECOMMENDATION_ARTICLE_LIMIT` cards | **[post-snapshot]** Deterministic local NLP ranks without article dates or read state as relevance features, excludes effectively read/dismissed candidates, displays escaped reasons, fingerprint-caches ranked IDs/reasons, and returns `never_cache` personalized HTML. |
...
| GET/POST* | `/feeds/` / `feeds` | session | `FeedForm`; every submitted website-or-feed URL is fetched and validated, then HTML autodiscovery links and bounded common feed paths are tried → HTML/redirect | Reads all feeds. Valid POST may call bounded feed fetches, rejects duplicate canonical feed URLs, then inserts Feed and message. |
...
| Session browser → mutation | **[F]** Django session auth and CSRF middleware/forms. Session cookie is HttpOnly, Secure in production, SameSite Lax. Every route declares an auth posture enforced by a URLconf-driven sweep, reading surfaces accept only safe methods, and every session mutation is proved to require CSRF and to commit nothing without it. | **[F]** The legacy digest stays deliberately method-agnostic behind CSRF per API-COMPAT-INV-001. **[D]** Posted `next` values are not same-origin validated on several handlers. |
...
| `RECOMMENDATION_ARTICLE_LIMIT` | Compose/settings default 30 | web | low | Must be an integer of at least 1 or settings import aborts. |
| `RECOMMENDATION_CACHE_SECONDS` | Compose/settings default 900 | web | low | Must be an integer of at least 1 or settings import aborts. Fingerprint changes invalidate earlier. |
