...
| GET* | `/recommended/` / `recommended` | session | Complete per-user SavedArticle profile and all unsaved Articles → up to `RECOMMENDATION_ARTICLE_LIMIT` cards | **[post-snapshot]** Deterministic local NLP ranks without article dates or read state, displays accurate read flags and escaped reasons, fingerprint-caches ranked IDs/reasons, and returns `never_cache` personalized HTML. |
