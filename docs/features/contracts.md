...
- **Normative current contract:** Recommended ranks every unread, unsaved Article for the authenticated user from that user's complete local SavedArticle history. Effective read state dismisses a candidate, while feed active state, publication time, fetched time, and article age do not remove or boost an unread candidate. Dates and read state are absent from relevance features and score. Results are deterministic for a fixed corpus/profile/read-state/dependency version, bounded by `RECOMMENDATION_ARTICLE_LIMIT`, deduplicated by canonical URL, and source-diversified only after personalized relevance scoring.
- **Scope / precedence:** local save intent is positive feedback even when Linkding delivery failed. Saved titles, notes, optional finite 0–5 interest scores, Article title/author/summary, Feed, and Category may influence relevance. Another user's saves and read state never enter the profile. Marking a recommendation read removes it immediately and keeps it absent on later loads; explicit unread can make it eligible again. Zero saves produce cold-start guidance rather than a generic recency/popularity list.
- **Executable evidence:** `test_recommendations.py` proves topical ranking across feeds, notes/rating handling, saved/read/duplicate exclusion, deterministic/date-free inputs, all-date unread eligibility, per-user isolation, profile/read-state fingerprint invalidation, immediate mark-read removal, escaped reasons, and cold-start/view behavior. `test_route_contracts.py` covers session auth and safe methods; `test_responsive_accessibility_browser.py` covers the R shortcut and responsive semantic route matrix.
