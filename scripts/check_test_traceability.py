...
INTRO_BASELINE = (
    "Current AST baseline: **82 feature IDs**, **48 invariant IDs**, "
    "**29 app test modules**, **433 app test methods**, and "
...
CONTRACTS_CURRENT_SUITE = (
    "This post-snapshot companion maps the **current suite: 29 test modules, "
    "433 test methods, and 2 expected failures**."
)
...
PINNED_TEXT = {
    ...
    "docs/features/contracts.md": (
        "**current suite: 29 test modules, 433 test",
...
def _stable_ids(root: Path) -> tuple[set[str], set[str]]:
    ...
    if len(features) != 82 or len(invariants) != 48:
        raise AssertionError(
