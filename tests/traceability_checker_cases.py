...
class TraceabilityCheckerMutationTests(unittest.TestCase):
    ...
    def test_recognizes_qualified_expected_failure(self) -> None:
        ...
        self.assert_rejected("expectedFailures=10")
    ...
    def test_rejects_stale_broad_expected_failure_count(self) -> None:
        self.mutate(
            ...
            "Exact broad gaps: 8 expected failures;",
        )
    ...
    def test_rejects_stale_second_contracts_expected_failure_count(self) -> None:
        self.mutate(
            ...
            "The current suite contains **8 expected failures**:",
        )
        self.assert_rejected("stale contracts repeated 9-expected-failures sentence")
    ...
    def test_rejects_regressed_ui_inv_005_validation_dimension(self) -> None:
        path = self.root / "docs/features/test-traceability.md"
        lines = path.read_text().splitlines()
        index = next(
        ...
        path.write_text("\n".join(lines) + "\n")
        self.assert_rejected("gap lacks an explicit Witan task slug")
